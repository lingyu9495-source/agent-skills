...
| `doubao-account-pool` | **豆包网页版多账号池** — Batch image/video generation on Doubao's free web quota, zero API keys: one account = one isolated browser profile (own `user-data-dir` + own CDP port) so accounts never collide or kick each other offline; QR-code takeover with auto-crop so the code can be scanned straight off a phone, parallel prompt feeding with a **baseline gate** that stops you from grabbing the previous run's stale image, a video flow that reads the confirmation card and **aborts on the paywall instead of clicking upgrade**, and a re-fetch path that recovers recent images/videos after the browser was closed. Config-driven (`env > ./doubao_config.json > $DOUBAO_HOME/config.json > built-in defaults`), ships no cookies, no keys, no machine paths; long jobs land on disk first and every command prints only `@@` structured lines, so page text / DOM / screenshots never flood the agent conversation. Keywords: 豆包 / 多账号 / 扫码接管 / 批量出图 / 出视频 / 浏览器自动化 / CDP / 零成本 |
## Install
