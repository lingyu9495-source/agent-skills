...
displayName: 豆包网页版多账号池·扫码接管+批量出图出视频（零API key）
description: 想用豆包网页版的免费额度批量出图、出视频，又不想办会员、不想申请 API key？这套把「一台机器养 N 个豆包账号 → 扫码接管 → 批量出图 → 出视频 → 取件 → 关掉浏览器还能回捞」封成配置驱动的命令行工具。一个账号 = 一个独立浏览器环境（独立 user-data-dir + 独立 CDP 端口），互不串号、互不踢下线；扫码接管的二维码自动裁切放大，可直接发手机扫；出图内置「基线闸」防止拿到上一轮的旧图；出视频先读确认卡再走，遇到付费弹层立即中止、绝不点升级；关掉浏览器后还能回捞最近会话里的图和视频，已生成没抓到链的可以单独重抓，不重跑、不白烧额度。所有命令只回 @@ 结构化短行，页面文本/DOM/截图绝不回流；长任务产物先落盘再筛。换机器、换智能体，只改配置就能跑——包内不含任何个人路径、cookie 与密钥，登录态在本机扫码产生。
version: 1.0.0
summary: 用豆包网页版免费额度批量出图出视频：一账号一独立浏览器环境、扫码接管、出图有基线闸、出视频过付费闸门、关掉浏览器还能回捞。零 API key，配置驱动，换机器只改配置。
license: MIT
author: 九品锦锂e
category: content-creation
tags: [doubao, 豆包, browser-automation, cdp, 出图, 出视频, 多账号, 浏览器自动化, 零成本, image-generation, video-generation, agent, automation]
---

# 豆包网页版 · 多账号池自动化（可移植包 v1.0）

> 把「一台机器养 N 个豆包账号 → 扫码接管 → 批量出图 → 出视频 → 取件」这套流程，
...
| 能力 | 说明 |
|:--|:--|
| 号位池 | 一个豆包账号 = 一个独立浏览器环境（独立 `user-data-dir` + 独立 CDP 端口），互不串号、互不踢下线 |
| 扫码接管 | 一条命令出二维码（自动裁切放大，可直接发手机扫），登录后自动校验 |
| 不打扰人 | 窗口挪到屏幕角落只露一小块 / 独立实例，**不抢焦点、不动用户自己的浏览器** |
| 批量出图 | 多号位并行投喂提示词、等结果、取高清原图（带「基线闸」防拿到旧图） |
| 出视频 | 切视频模式 → 上传垫图 → 注入提示词 → 发送 → **读确认卡过付费闸门** → 等完成 → 抓链下载 |
| 回捞 | 浏览器关掉之后重新上线取结果（恢复最近会话 → 抓图/抓视频） |
| 零污染 | 所有命令只打印 `@@` 开头的结构化短行，**页面文本/DOM/截图 base64 绝不回流主对话** |

**不包含**：账号 cookie、登录态文件、任何 API key、任何付费接口。号位登录态由使用者在本机扫码产生。

## 1. 安装（30 秒）

```bash
pip install -r requirements.txt        # websocket-client / pillow
python scripts/doubao_cli.py doctor    # 自检：浏览器路径 / 依赖 / 状态目录 / 已有号位
```
...
| 环境变量 | 含义 | 默认 |
|:--|:--|:--|
| `DOUBAO_HOME` | 状态根目录（每个号位一个子目录） | `~/.doubao_pool` |
...
| `DOUBAO_PORT_BASE` | 号位基端口（号位 n → base+n） | `9230` |
| `DOUBAO_MAX_SLOT` | 号位上限 | `20` |
...
| 命令 | 用途 |
|:--|:--|
| `doctor` | 环境自检（浏览器/ffmpeg/依赖/状态目录/号位） |
| `status [--deep]` | 号位在线与登录态一览 |
| `up N...` / `down N...` / `down --all` | 起停号位（**只杀本号位进程树**） |
| `login N` | 开登录弹窗 + 取二维码（裁切放大 PNG，可直接发手机）+ 失效复活检查 |
| `check N` | 登录态三重校验（URL / 无登录按钮 / 有历史会话） |
| `nick N` | 读该号账号昵称 |
| `stealth N` | 反检测体检（webdriver / visibility / outer 尺寸 / cdc 痕迹 / WebGL） |
...
| `img-batch --spec spec.json --out DIR` | 多号位并行批量出图，产出 `result.json` |
| `vid --slot N --image 垫图.png --file 词.txt --out DIR` | 出视频全流程（含付费闸门） |
| `vid-regrab --slot N --out DIR [--name 名]` | 已生成但没抓到链的重抓（**不重跑生成、不白烧额度**） |
| `fetch --slot N --out DIR` | 回捞最近会话的图/视频 |
| `trim --slot N [--keep K]` | 关掉该号位多余标签页，只留 K 个（默认 1） |

## 4. 五条主流程

### 4.1 接管一个新账号（加号 = 加免费额度）
```bash
python doubao_cli.py status                 # 先看谁在线
python doubao_cli.py up 6                   # 永远用「下一个空号」，别复用已登录号位目录
...
- **号位分配铁律**：永远用下一个空号。复用别人的号位目录会把 cookie 混进来，两个账号互踢。
...
**付费闸门**（见「专业版/订阅/付费额度」立即中止，不许点任何升级按钮）→ 确认 → 等完成 →
抓视频链 → 下载 → 规格核对（必须有音轨）。
...
```
**号位池生命周期只归调度方（主脑）**：把批量任务派给子代理时，任务书必须写死「只准用分到的号位，**禁止 up、禁止 down**」，收工由调度方统一关。子代理跑完就 down 会关掉别人正在跑的号位。

## 5. 风控与红线（违反 = 号废）

1. **提交生成后固定等 5 分钟**再回来看，禁止秒级轮询、反复刷新、连点——密集操控是风控头号触发源。
2. **同一动作连败 3 次立即停手**，不要硬试。
3. 多号轮转**错开节奏**（起跑间隔 0.4s+），不要 N 个号同时狂点。
4. 只走「填入 → 提交 → 等 → 取结果」主路径，不在页面上乱逛乱试。
5. **下拉/弹层是开关式的**：点一次开、再点同一处就关。连续「点账号 → 点菜单项」会自己把菜单关掉，症状是「菜单里什么都找不到」。正解＝打开一次，之后只点弹层内部文字。
6. **付费闸门优先于一切**：确认卡/弹层出现「专业版 / 订阅 / 付费额度 / 升级」字样 → 立即中止该号位，绝不点升级。
7. **只杀自己**：`down` 只按 pidfile + 命令行含本号位目录做双重校验后杀进程树，**永不** `taskkill /IM chrome.exe`。

完整坑清单见 [references/pitfalls.md](references/pitfalls.md)，风控细则见 [references/risk-control.md](references/risk-control.md)。
...
关键设计（照抄别改）：**① 只回 `@@` 短行；② 页面文本/DOM/图片 base64 绝不回流；③ 长任务产物先落盘再筛；④ 号位生命周期只归调度方。**
...
> 使用中遇到问题、想要定制版本或需要配套支持，可通过上面任一方式联系。
