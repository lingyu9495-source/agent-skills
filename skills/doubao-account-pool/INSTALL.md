...
## 1. 装法 A：Hermes Agent（原生 skill）

把本目录整个拷到 Hermes 的 skills 目录下：

```bash
# 默认 profile
cp -r doubao-account-pool "$LOCALAPPDATA/hermes/skills/"
# 指定 profile（Windows）
cp -r doubao-account-pool "$LOCALAPPDATA/hermes/profiles/<profile>/skills/"
```
...
**验证**：
```bash
cd "$LOCALAPPDATA/hermes/skills/doubao-account-pool/scripts"
...
```bash
export DOUBAO_HOME="$HOME/.doubao_pool"      # 状态目录：号位 profile + 产出
...
所有号位登录态都在它下面，备份它 = 备份所有账号登录状态（**别提交到 git**）。
...
{
  "name": "doubao_pool",
  "description": "操控豆包网页版多账号池。所有命令只返回 @@ 开头的结构化短行。",
  "parameters": {
    "type": "object",
    "properties": {
      "command": {"type": "string", "enum": ["doctor","status","up","down","login","check","nick","stealth","visible","ask","img","img-batch","vid","vid-regrab","fetch","trim"]},
...
1. 调度方（主脑）先 `status` 看号位，再按号位切分任务；
2. 任务书里写死「**只准用分给你的号位，禁止 up、禁止 down、禁止整屏截图、禁止 dump DOM**」；
3. 执行方只回 `@@` 短行 + 自己的产物路径；
...
python doubao_cli.py status            # ② 应逐号位输出 @@SLOT n port=... alive=YES/no logged=...
python doubao_cli.py up 1              # ③ 起一个号位（应见 @@UP slot=1 port=... 与 @@READY）
...
| `outerWidth/outerHeight = 0` | 窗口被挪到屏幕外，自动化指纹极强 | 同上，改用「屏内露一小块」而不是挪出屏 |
| 注入成功、豆包一个字不回 | 免费额度耗尽（静默拒答） | 换号位；连续多号位全静默才判额度见底 |
...
| `down` 返回码 0 但号位没关掉（stderr 有 `UnicodeDecodeError` / `_readerthread`） | 中文 Windows + `PYTHONUTF8=1`：`powershell`/`taskkill` 输出是 GBK，被按 UTF-8 解码，异常在子线程里被吞掉 | 已在本包修复（所有 `subprocess.run` 显式带 `encoding='utf-8', errors='replace'`）。**如果你自己扩了调用，照抄这个参数** |
...
```bash
python doubao_cli.py down --all          # 先关掉所有号位
